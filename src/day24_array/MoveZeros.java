...
Hint: You don’t have to manipulate the given array, try to use another array too. Think about the indexes
    ...
    public static void main(String[] args) {
        ...
        int[] fixed = new int[nums.length]; // [0, 0, 0, 0, 0, 0,0]
        int indexToRed = 0; // read every element from beginning to end
        int indexToStore = 0; // store values into the fixed array, maybe will not increment each time

        //indexToRead : 0
        //indexToStore : 0

        while (indexToRed < nums.length) {
            if (nums[indexToRed] != 0) {
                fixed[indexToStore] = nums[indexToRed];
                indexToStore++;
            }
            indexToRed++;
        }
